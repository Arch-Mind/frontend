import { FileContribution } from '../api/types';

export type HeatmapMode = 'off' | 'commit_count' | 'last_modified' | 'author_count';
...
export function buildHeatmap(contributions: FileContribution[], mode: HeatmapMode): HeatmapState {
    if (mode === 'off') {
...
export function getHeatmapColor(metric: number, maxMetric: number): string {
    if (maxMetric <= 0) {
...
export function getMetric(contribution: FileContribution, mode: HeatmapMode): number {
    switch (mode) {
        case 'commit_count':
...
export function formatHeatmapTooltip(contribution: FileContribution): string {
...
export function formatHeatmapMetric(mode: HeatmapMode, metric: number): string {
