...
export class ArchMindApiClient {
    ...
    private loadConfig(): ApiClientConfig {
        ...
        return {
            gatewayUrl: (config.get<string>('backendUrl', 'http://localhost:8080') ?? '').replace(/\/+$/, ''),
            graphEngineUrl: (config.get<string>('graphEngineUrl', 'https://graph-engine-production-90f5.up.railway.app') ?? '').replace(/\/+$/, ''),
    ...
    public async triggerArchitectureAnalysis(
        ...
        return this.request<ArchitectureInsightsResponse>(
            this.config.gatewayUrl,
            `/api/analyze/${encodeURIComponent(repoId)}/architecture${refresh ? '?refresh=true' : ''}`,
            { method: 'POST' },
            180_000
        );
    }
    ...
    private transformGraphData(
        ...
        const nodes: TransformedNode[] = graphData.nodes.map((node, _index) => {
            const props = node.properties || {};
